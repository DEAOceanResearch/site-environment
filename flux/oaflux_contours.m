...
H(1:3)=contour_oaflux(files,'evapr', years, 'Evaporation rate (cm/year)', ...
		'Evaporation',0.1,RANGE);

files=[ SITE_ENVIRONMENT_DIR 'lh_oaflux.nc'];
H(end+[1:3])=contour_oaflux(files,'lhtfl', years, 'Monthly Mean (W/m^2)', ...
		'Latent Heat Flux',0.1,RANGE);

files=[ SITE_ENVIRONMENT_DIR 'sh_oaflux.nc'];
H(end+[1:3])=contour_oaflux(files,'shtfl', years,'Monthly Mean (W/m^2)', ...
		'Surface Sensible Heat Flux',0.1,RANGE);

files=[ SITE_ENVIRONMENT_DIR 'qa_oaflux.nc'];
H(end+[1:3])=contour_oaflux(files,'hum2m', years,'Monthly Mean (g/kg)', ...
		'Specific Humidity at 2m',0.01,RANGE);

files=[ SITE_ENVIRONMENT_DIR 'ws_oaflux.nc'];
H(end+[1:3])=contour_oaflux(files,'wnd10', years,'Monthly Mean (m/s)', ...
		'Neutral Wind Speed at 10m',0.01,RANGE);


files=[ SITE_ENVIRONMENT_DIR 'ts_oaflux.nc'];
H(end+[1:3])=contour_oaflux(files,'tmpsf', years,'Monthly Mean ( ^\circ{C})', ...
		'Sea Surface Temperature',0.01,RANGE);

files=[ SITE_ENVIRONMENT_DIR 'ta_oaflux.nc'];
H(end+[1:3])=contour_oaflux(files,'tmp2m', years,'Monthly Mean ( ^\circ{C})', ...
...
H(end+[1:3])=contour_oaflux(files,'qnet', years, 'Monthly Mean (W/m^2)', ...
		'Net Surface Heat Flux',0.1,RANGE);

files=[ SITE_ENVIRONMENT_DIR 'lw_isccp.nc'];
H(end+[1:3])=contour_oaflux(files,'nlwrs', years, 'Monthly Mean (W/m^2)', ...
		'Net Surface Longwave Radiation Flux',0.1,RANGE);

files=[ SITE_ENVIRONMENT_DIR 'sw_isccp.nc'];
H(end+[1:3])=contour_oaflux(files,'nswrs', years, 'Monthly Mean (W/m^2)', ...
